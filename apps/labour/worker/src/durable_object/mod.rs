...
impl DurableObject for LabourAggregate {
    ...
    async fn websocket_message(
        ...
        let result = handle_websocket_command(&self.services, command.command, user);
        ...
        match result {
            Ok(_) => {
                ...
                if let Err(e) = ws.send_with_str(&response.to_string()) {
            ...
            Err(e) => {
                ...
                if let Err(send_err) = ws.send_with_str(&response.to_string()) {
                    error!(error = ?send_err, "Failed to send error response");
                }
            }
        }
