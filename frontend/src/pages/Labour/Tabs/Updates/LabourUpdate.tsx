...
export function LabourUpdate({data, completed, owner}: {data: LabourUpdateDTO, completed: boolean, owner: boolean}) {
    ...
    return (
        ...
        <Group>
            ...
            <Text>{announce && '📣' || '💫'}</Text>
            <Badge visibleFrom='xs' variant="filled" size="md" radius="md" bg={announce && 'var(--mantine-color-pink-6)' || '#24968b'}>
              {data.labour_update_type.split("_")[0]}
            </Badge>
            <Badge hiddenFrom='xs' variant="filled" size="sm" radius="md" bg={announce && 'var(--mantine-color-pink-6)' || '#24968b'}>
              {data.labour_update_type.split("_")[0]}
            </Badge>
