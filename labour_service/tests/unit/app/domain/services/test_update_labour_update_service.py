...
def test_can_update_labour_update_message(sample_labour: Labour):
    ...
    assert len(labour.labour_updates) == 1
    labour_update = copy(labour.labour_updates[0])
    ...
    assert len(labour.labour_updates) == 1
    updated_labour_update = labour.labour_updates[0]
...
def test_cannot_announce_an_announcement(sample_labour: Labour):
    ...
    with pytest.raises(CannotUpdateAnnouncement):
