...
@pytest.fixture
def sample_event():
    """Fixture for a sample DomainEvent."""
    return DomainEvent(
        id="evt-123",
...
def test_get_topic_path(producer: PubSubEventProducer, sample_event: DomainEvent):
...
def test_serialize_event(producer: PubSubEventProducer, sample_event: DomainEvent):
...
async def test_publish_success(
    ...
    # Mock run_in_executor to immediately return the future's result
...
async def test_publish_general_exception(
    ...
    async def mock_executor_value_error():
