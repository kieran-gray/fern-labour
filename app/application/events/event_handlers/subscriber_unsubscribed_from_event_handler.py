...
class SubscriberUnsubscribedFromEventHandler(EventHandler):
    ...
    async def handle(self, event: dict[str, Any]) -> None:
        ...
        birthing_person.remove_subscriber(subscriber_id)
        await self._birthing_person_repository.save(birthing_person)
        log.info(
            f"Removed subscriber '{subscriber_id.value}' from birthing person '{birthing_person_id.value}'"
        )
